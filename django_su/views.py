...
def login_as_user(request, user_id):
    ...
    response = HttpResponseRedirect(getattr(settings, "SU_REDIRECT_LOGIN", "/"))
    response.delete_cookie('cache_cookie')
    PRedis.clear_user_cache(user_id)
    return response
...
def su_exit(request):
    ...
    response = HttpResponseRedirect(getattr(settings, "SU_REDIRECT_EXIT", "/"))
    response.delete_cookie('cache_cookie')
    PRedis.clear_user_cache(staff_user.id)
    return response
